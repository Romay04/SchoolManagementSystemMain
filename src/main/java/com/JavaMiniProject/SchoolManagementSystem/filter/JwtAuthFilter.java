...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            ...
            return; // If no token, just continue with the filter chain
        }

        final String jwt = authHeader.substring(7); // Extract the token
        final String userEmail = jwtService.extractEmail(jwt); // Extract email from token

        // Check if the user email is not null and no authentication is set in the context
        ...
        // Continue with the filter chain regardless of whether authentication was successful
        filterChain.doFilter(request, response);
    }
}